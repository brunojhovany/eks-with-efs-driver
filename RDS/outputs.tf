...
locals {
  postgres_pod_test = <<POD
apiVersion: batch/v1
kind: Job
metadata:
  name: postgres-isready
  labels:
    job-name: postgres-isready
spec:
  template:
    spec:
      containers:
      - name: postgres-isready
        ...
        command: ['pg_isready', '-h', '${module.db.db_instance_address}', '-U','${module.db.db_instance_username}', '-t', '15']
      restartPolicy: OnFailure
  POD
}

resource "local_file" "postgres_pod_test" {
  filename        = "${path.module}/externals/job.yaml"
  content         = local.postgres_pod_test
  file_permission = "440"
}